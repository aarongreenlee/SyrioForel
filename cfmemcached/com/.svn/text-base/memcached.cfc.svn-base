	...
	<cffunction name="init" displayname="init" hint="init function" access="public" output="false" returntype="any">
		...
		<cfscript>
			...
			*/
			...
			var javaLoader = createObject("component", "util.JavaLoader").init(
				facadeFactory.getServerFacade(),
				MemcachedClientJavaJarDir
			);
			
			variables.addrUtil = javaLoader.create("net.spy.memcached.AddrUtil").init();
			
			variables.memcached = javaLoader.create("net.spy.memcached.MemcachedClient").init(
				addrUtil.getAddresses(arguments.servers)
			);
